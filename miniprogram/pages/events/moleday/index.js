...
Page({
  ...
  deleteQuestion(event){
    ...
    db.collection('moleday').doc(QuestionId)
    .remove()
    .then(res=>{
  ...
  checkIfVerified(){
    ...
    .get({
      success: function (res) {
        ...
        if (res.data[0].moledayVerified){
          //scavenger hunt progress
          let scav = [];
          let exp = [];
          for (let i=0; i<38; i++){
            if (res.data[0].moledayAnswers["scav-" + i.toString()] == null){
              scav.push(0);
            }
            else if (res.data[0].moledayAnswers["scav-" + i.toString()] >= 0){
              scav.push(1);
            }
          }

          for (let i=1; i<=13; i++){
            if (res.data[0].moledayAnswers["exp-" + i.toString()] == null){
              exp.push(0);
            }
            else if (res.data[0].moledayAnswers["exp-" + i.toString()] >= 0){
              exp.push(1);
            }
          }

          that.setData({
            moledayVerified:true,
            ScavProgress: scav,
            loading:false,
      ...
      success (res) {
        ...
        for (let i =0; i<codes.length; i++){
          if (codes[i] == res.result){
            ...
            wx.navigateTo({
              url: "../moleday/scavenger hunt/index"
            })
