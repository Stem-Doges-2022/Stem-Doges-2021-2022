...
Page({
  ...
  getRanking() {
    ...
    let people = [];
    db.collection('userInfo_bak5').where({bioweekVerified:true}).count({
      success: function(res) {
        for (let i=0 ; i<res.total; i+=20){
          db.collection('userInfo_bak5').where({bioweekVerified:true}).skip(i).get({
            success: function(res) {
              people = people.concat(res.data);

              people.sort((a, b) => b.bioweekPoints - a.bioweekPoints)
              ...
              for (let j=0;j<people.length; j++){
                console.log((j+1).toString() +" " + people[j].userRealName +" "+ people[j].userGrade + "(" + people[j].userClass + ")" + " "+people[j].bioweekPoints);
              }
            }
          })
        }
